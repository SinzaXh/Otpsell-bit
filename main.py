...
CONFIG = {
    ...
    "ADMIN_IDS": [
        int(x.strip())
        for x in os.getenv("ADMIN_IDS", "8251818467,6936153954").split(",")
        if x.strip()
    ],
    "FORCE_JOIN_USERNAME":
...
async def check_balance_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        ...
        f"Need to add balance? Contact @{CONFIG['OWNER_HANDLE']}",
        reply_markup=InlineKeyboardMarkup(kb),
        parse_mode="Markdown")
...
async def country_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user['balance'] < price:
        await q.edit_message_text(
            f"❌ **Insufficient Balance**\n\nRequired: ₹{price}\nYour Balance: ₹{user['balance']}\n\nContact @{CONFIG['OWNER_HANDLE']} to add balance.",
            parse_mode="Markdown")
        return
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
...
async def monitor_telegram_messages(context: ContextTypes.DEFAULT_TYPE,
                                    user_id: int, acc_id: int, phone: str,
                                    session_path: str):
    """
    ...
    async def handler(event):
        try:
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                cur = await db.execute(
                    "SELECT two_fa_password FROM accounts WHERE id=?",
                    (acc_id, ))
                row = await cur.fetchone()
                if row:
                    two_fa = row[0]
            ...
            try:
                await context.bot.send_message(chat_id=user_id,
                                               ...
                                               parse_mode="Markdown",
                                               reply_markup=kb)
                logger.info("Forwarded OTP %s to %s (acc %s)", otp, user_id,
                            acc_id)
            except Exception as e:
