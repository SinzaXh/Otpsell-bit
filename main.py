...
# ============================
CONFIG = {
    "BOT_TOKEN": "8377731771:AAGhDHE1ETtZutmlLmx10PzGvugZAofhh-I",
    "API_ID": 35738244,
    "API_HASH": "506bdae7bb4d3379ed106237d36e80f4",
    "TWO_FA_PASSWORD": None,
    "ADMIN_IDS": [8251818467, 6936153954],
    "FORCE_JOIN_USERNAME": "@abouttechyrajput",
    "FORCE_JOIN_CHAT_ID": -1002731834108,
    "DATABASE_PATH": "shop.db",
    "SESSION_DIR": "sessions",
    "COUNTRY_PRICES": {
        "US": 40.0, "ET": 35.0, "VN": 35.0, "IN": 40.0, "NP": 40.0, "SV": 55.0, "PH": 80.0
    },
    "OWNER_HANDLE": "abouttechyrajput",
    "DEVELOPER_CREDITS": "🤖 Developed by @BSRAJPUT 0",
    "RESERVE_MINUTES": 10,
    # timeouts (seconds)
...
async def get_user(user_id: int, username: Optional[str]):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT id, username, balance FROM users WHERE id=?", (user_id,))
...
def join_buttons() -> InlineKeyboardMarkup:
    ...
    if uname and uname.startswith("@"):
        return InlineKeyboardMarkup([[InlineKeyboardButton(f"✅ Join {uname}", url=f"https://t.me/{uname[1:]}")]])
...
async def release_expired_reservations_tick():
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT id, metadata FROM accounts WHERE status='reserved'")
        ...
        for acc_id, meta in rows:
            try:
                ...
                if ru_dt < datetime.now(IST):
                    await db.execute("UPDATE accounts SET status='available', metadata=NULL WHERE id=?", (acc_id,))
                    changed += 1
            except Exception:
                await db.execute("UPDATE accounts SET status='available', metadata=NULL WHERE id=?", (acc_id,))
                changed += 1
...
Click the button below to join, then tap '✅ Verify Join' to continue.
"""
        kb = [
            [InlineKeyboardButton("📢 Join Channel", url=f"https://t.me/{CONFIG['FORCE_JOIN_USERNAME'][1:]}")],
            [InlineKeyboardButton("✅ Verify Join", callback_data="verify_join")]
        ]
        await update.message.reply_text(join_text, reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
        return
...
async def verify_join_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await check_force_join(q.from_user.id, context.application):
        await q.edit_message_text("❌ **You haven't joined the channel yet!**\n\nPlease join and then verify.",
...
🇵🇭 Philippines - ₹80
    """
    kb = []
    kb.append([InlineKeyboardButton("🛒 Buy Accounts", callback_data="buy_accounts")])
    kb.append([InlineKeyboardButton("💰 Check Balance", callback_data="check_balance")])
    if is_admin:
        kb.append([InlineKeyboardButton("⚡ Admin Panel", callback_data="admin_panel")])
    kb.append([InlineKeyboardButton("👨‍💻 Owner Contact", url=f"https://t.me/{CONFIG['OWNER_HANDLE']}")])
    if update.message:
...
🇵🇭 Philippines - ₹80
    ...
    if is_admin:
        kb.append([InlineKeyboardButton("⚡ Admin Panel", callback_data="admin_panel")])
    kb.append([InlineKeyboardButton("👨‍💻 Owner Contact", url=f"https://t.me/{CONFIG['OWNER_HANDLE']}")])
    await q.edit_message_text(welcome_text, reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")

async def buy_accounts_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    if not await check_force_join(q.from_user.id, context.application):
        await q.edit_message_text("📢 Please join our channel to continue using this bot:", reply_markup=join_buttons())
        return
    countries = [("US", 40), ("ET", 35), ("VN", 35), ("IN", 40), ("NP", 40), ("SV", 55), ("PH", 80)]
    kb = []
    for country, price in countries:
        kb.append([InlineKeyboardButton(f"{country_flag(country)} {country} ₹{price}", callback_data=f"country_{country}")])
    kb.append([InlineKeyboardButton("🔙 Back to Main", callback_data="main_menu")])
    await q.edit_message_text("🌍 **Choose a Country**\n\nSelect your preferred country:", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
...
async def country_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user['balance'] < price:
        await q.edit_message_text(f"❌ **Insufficient Balance**\n\nRequired: ₹{price}\nYour Balance: ₹{user['balance']}\n\nContact @{CONFIG['OWNER_HANDLE']} to add balance.", parse_mode="Markdown")
        return
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT id, phone_number, price FROM accounts WHERE country_code=? AND status='available' LIMIT 1", (cc,))
        row = await cur.fetchone()
    if not row:
        await q.edit_message_text(f"❌ **No {country_flag(cc)} {cc} numbers available**\n\nPlease check back later.", parse_mode="Markdown")
        return
    acc_id, phone, price_db = row
    meta = {"reserved_by": q.from_user.id, "reserved_at": now_iso(), "reserved_until": minutes_from_now(CONFIG["RESERVE_MINUTES"])}
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE accounts SET status='reserved', metadata=? WHERE id=?", (json.dumps(meta), acc_id))
        await db.commit()
    kb = [[InlineKeyboardButton("➡️ Continue", callback_data=f"getotp_{acc_id}")], [InlineKeyboardButton("🔙 Choose Another Country", callback_data="buy_accounts")]]
    await q.edit_message_text(f"📱 **TRY LOGIN**\n\n📞 **Number:** `{phone}`\n\nInstructions:\n1. Copy the number\n2. Paste in Telegram app\n3. Tap Continue\n4. I will forward the OTP here automatically\n\n🔄 Waiting for OTP...\n\n_WHEN YOU HAVE SUCCESSFULLY LOGGED IN TAP DONE ✅_", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [
        ...
        [InlineKeyboardButton("📊 Stats", callback_data="admin_stats"), InlineKeyboardButton("📇 Accounts", callback_data="admin_accounts")],
        [InlineKeyboardButton("💳 Manage Balance", callback_data="check_balance")],
...
async def admin_upload_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = []
    for country, price in countries:
        kb.append([InlineKeyboardButton(f"{country_flag(country)} {country}", callback_data=f"admin_country_{country}")])
...
async def admin_country_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"📥 **Upload Account — {cc}**\n\nEnter the phone number (international, e.g. +911234567890) for the account. You'll receive a code in Telegram; paste it here.", parse_mode="Markdown")
...
async def handle_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not phone.startswith("+") or len(phone) < 8:
        await update.message.reply_text("Please send a valid phone number in international format (e.g. +911234567890).")
        return
...
async def handle_otp_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    """
    admin_id = update.effective_user.id
    if admin_id not in PENDING_UPLOADS or PENDING_UPLOADS[admin_id].get("step") not in ("waiting_otp",):
        await update.message.reply_text("No pending phone verification. Use /upload to start.")
        return
    ...
    if not otp:
        await update.message.reply_text("Please send the numeric OTP you received (e.g. 45441).")
        return
    ...
    if pending["otp_attempts"] > 5:
        ...
        await update.message.reply_text("❌ Too many invalid OTP attempts. Upload cancelled and session removed.")
        return
    ...
    try:
        ...
        try:
            ...
            if not pending.get("sent_2fa_prompt"):
                ...
                await update.message.reply_text("🔒 Awaiting 2FA password (previously requested).")
            try:
                await client.disconnect()
            except Exception:
                pass
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute(
                ...
                (pending.get("country", "US"), phone, session_fname, admin_id, "available", CONFIG["COUNTRY_PRICES"].get(pending.get("country", "US"), 40.0), json.dumps({}))
            )
            await db.commit()
...
async def handle_2fa_password_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('upload_after_otp_waiting_choice'):
        if txt.lower() == "skip":
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute("UPDATE accounts SET two_fa_password=NULL WHERE id=?", (acc_id,))
                await db.commit()
            ...
            session_path = os.path.join(SESSION_DIR, session_fname) if session_fname else None
            async with aiosqlite.connect(DB_PATH) as db:
                if acc_id:
            ...
            try:
                ...
                pass
            context.user_data.pop('upload_after_otp_waiting_choice', None)
            await update.message.reply_text("✖️ Upload canceled and session removed.")
            return
        else:
            ...
            phone = context.user_data.get('upload_after_otp_phone')
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute("UPDATE accounts SET two_fa_password=? WHERE id=?", (password, acc_id))
                await db.commit()
            context.user_data.pop('upload_after_otp_waiting_choice', None)
            await update.message.reply_text(f"✅ Account Added Successfully!\n\n📱 Number: {phone}\n🔒 2FA: Password set\n\nThe account is now available for sale! 🎊")
            return
    ...
    if pending and pending.get("step") == "waiting_2fa":
        ...
        if txt.lower() == "cancel":
            ...
            await update.message.reply_text("✖️ Sign-in aborted. Session removed.")
            return
        ...
        if pending["2fa_attempts"] > 5:
            ...
            PENDING_UPLOADS.pop(admin_id, None)
            context.user_data.pop('waiting_2fa', None)
            await update.message.reply_text("❌ Too many incorrect 2FA attempts. Upload cancelled and session removed.")
            return
        ...
        try:
            ...
            try:
                ...
                await update.message.reply_text("❌ Incorrect 2FA password. Please try again or send 'cancel' to abort.")
                try:
                    await client.disconnect()
                except Exception:
                    pass
            ...
            async with aiosqlite.connect(DB_PATH) as db:
                await db.execute(
                    ...
                    (pending.get("country", "US"), phone, session_fname, password, admin_id, "available", CONFIG["COUNTRY_PRICES"].get(pending.get("country", "US"), 40.0), json.dumps({}))
                )
                await db.commit()
                cur = await db.execute("SELECT last_insert_rowid()")
                row = await cur.fetchone()
            ...
            await update.message.reply_text(f"❌ Error finishing 2FA sign-in: {e}")
            try:
                await client.disconnect()
            except Exception:
                pass
    ...
    # fallback
    await update.message.reply_text("I didn't understand that. Use /upload to start an account upload.")
...
async def monitor_telegram_messages(context: ContextTypes.DEFAULT_TYPE, user_id: int, acc_id: int, phone: str, session_path: str):
    """
    Start a one-shot Telethon client listening to chat 777000 (both incoming and outgoing messages).
    On first match of OTP, forward the OTP (and optionally 2FA password) to the user and stop.
    """
    client = TelegramClient(session_path, CONFIG["API_ID"], CONFIG["API_HASH"])
    try:
        await client.connect()
    except Exception as e:
        logger.error("Monitor: failed to connect Telethon client: %s", e)
        return

    import re
    primary_re = re.compile(r'Login Code:\s*(\d{5})', re.IGNORECASE)
    ...
    async def handler(event):
        try:
            ...
            logger.info("Monitor saw message in 777000: %r", text)
            m = primary_re.search(text)
            if m:
                otp = m.group(1)
            else:
                m2 = fallback_re.search(text)
                otp = m2.group(1) if m2 else None
            if not otp:
                return
            # fetch account two_fa from DB
            two_fa = None
            async with aiosqlite.connect(DB_PATH) as db:
            ...
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Get New OTP", callback_data=f"getotp_{acc_id}"), InlineKeyboardButton("✅ Done", callback_data=f"done_{acc_id}")]])
            try:
                await context.bot.send_message(chat_id=user_id, text=send_text, parse_mode="Markdown", reply_markup=kb)
                logger.info("Forwarded OTP %s to %s (acc %s)", otp, user_id, acc_id)
            except Exception as e:
                logger.error("Failed to forward OTP message: %s", e)
            forwarded_event.set()
        except Exception as e:
            logger.exception("Exception in monitor handler: %s", e)

    # wait for forwarded_event or timeout
    try:
        await asyncio.wait_for(forwarded_event.wait(), timeout=600)
    except asyncio.TimeoutError:
        logger.info("Monitor timed out waiting for OTP for acc %s", acc_id)
        try:
            await client.disconnect()
        except Exception:
            pass
...
async def get_otp_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # get account
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT phone_number, session_file, two_fa_password FROM accounts WHERE id=?", (acc_id,))
        row = await cur.fetchone()
    if not row:
    ...
    await q.edit_message_text(f"📱 **TRY LOGIN**\n\n📞 **Number:** `{phone}`\n\nSteps:\n1. Paste number in Telegram app\n2. Tap Continue\n\n🔄 **Monitoring for OTP...**\n\n_When you receive OTP it will be forwarded here._", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
    # start monitor in background
    asyncio.create_task(monitor_telegram_messages(context, q.from_user.id, acc_id, phone, session_path))
...
async def done_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = await get_user(q.from_user.id, q.from_user.username)
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT phone_number, price, session_file, country_code FROM accounts WHERE id=?", (acc_id,))
        row = await cur.fetchone()
    if not row:
        await q.answer("Account not found!", show_alert=True)
        return
    ...
    # check balance
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT balance FROM users WHERE id=?", (q.from_user.id,))
        row = await cur.fetchone()
    ...
    if bal < price:
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("UPDATE accounts SET status='available', metadata=NULL WHERE id=?", (acc_id,))
            await db.commit()
        await q.edit_message_text(f"❌ **Insufficient Balance**\n\nRequired: ₹{price}\nYour Balance: ₹{bal}\n\nAccount released. Please add balance and try again.", parse_mode="Markdown")
        return
    # deduct and mark as sold
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("UPDATE users SET balance = balance - ? WHERE id=?", (price, q.from_user.id))
        await db.execute("UPDATE accounts SET status='sold', metadata=NULL WHERE id=?", (acc_id,))
        await db.execute("INSERT INTO transactions (user_id, account_id, amount, type) VALUES (?, ?, ?, 'purchase')", (q.from_user.id, acc_id, price))
        await db.commit()
    # notify admins
    for admin_id in CONFIG["ADMIN_IDS"]:
        try:
            await context.bot.send_message(admin_id, f"💰 New sale: Buyer {user['username'] or user['id']}\nNumber: {phone}\nAmount: ₹{price}", parse_mode="Markdown")
        except Exception:
            pass
    kb = [[InlineKeyboardButton("🛒 Buy Another", callback_data="buy_accounts")]]
    await q.edit_message_text(f"🎉 **Purchase Successful!**\n\n📱 **Number:** `{phone}`\n💰 **Amount Paid:** ₹{price}\n\nThank you for your purchase!", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")

# ---------- Admin commands (complete) ----------
@admin_only
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT status, COUNT(*) FROM accounts GROUP BY status")
        status_rows = await cur.fetchall()
        cur = await db.execute("SELECT country_code, status, COUNT(*) FROM accounts GROUP BY country_code, status ORDER BY country_code, status")
        country_rows = await cur.fetchall()
        cur = await db.execute("SELECT COUNT(*) FROM users")
        user_count = (await cur.fetchone())[0]
        cur = await db.execute("SELECT SUM(amount) FROM transactions WHERE type='purchase'")
...
async def cmd_accounts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute("SELECT id, country_code, phone_number, status, price FROM accounts ORDER BY status, country_code")
        rows = await cur.fetchall()
    if not rows:
...
FOOTER = f"\n\n{CONFIG['DEVELOPER_CREDITS']} • @{CONFIG['OWNER_HANDLE']}"

async def send_admin_reply(update: Update, text: str):
    text_with_footer = f"{text}{FOOTER}"
    if update.callback_query:
        try:
            await update.callback_query.edit_message_text(text_with_footer, parse_mode="Markdown")
            return
        except Exception:
            pass
    if update.message:
        await update.message.reply_text(text_with_footer, parse_mode="Markdown")
    else:
        await update.effective_chat.send_message(text_with_footer, parse_mode="Markdown")

@admin_only
async def cmd_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    args = context.args or []
    if not args:
        await send_admin_reply(update, "Usage:\n/balance <user_id_or_username>\n/balance set <user_id_or_username> <amount>")
        return
    if args[0].lower() == "set":
        if len(args) < 3:
            await send_admin_reply(update, "Usage: /balance set <user_id_or_username> <amount>")
            return
        target = args[1]; amount = args[2]
        try:
            amt = float(amount)
        except Exception:
        ...
        if not user_id:
            ...
            return
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute("UPDATE users SET balance=? WHERE id=?", (amt, user_id))
            await db.commit()
            cur = await db.execute("SELECT balance FROM users WHERE id=?", (user_id,))
            new = (await cur.fetchone())[0]
        await send_admin_reply(update, f"✅ Balance set for user {user_id}: ₹{new}")
        return
    # show balance
    target = args[0]; user_id = None
    if target.isdigit():
        user_id = int(target)
    else:
        username = target.lstrip("@")
...
async def cmd_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in user_ids:
        try:
            await context.bot.send_message(uid, f"{msg}{FOOTER}", parse_mode="Markdown")
            sent += 1
            await asyncio.sleep(0.05)
        except Exception:
            failed += 1
    await send_admin_reply(update, f"Broadcast done. Sent: {sent}, Failed: {failed}")
...
async def cmd_ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not user_id:
        ...
        return
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("INSERT INTO bans (user_id, reason) VALUES (?, ?)", (user_id, "Admin ban"))
        await db.commit()
...
async def cmd_unban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not context.args:
        ...
        return
    target = context.args[0]; user_id = None
...
async def cmd_addcoins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 2:
        await update.message.reply_text("Usage: /addcoins <username_or_userid> <amount>")
        return
    target = context.args[0]
    try:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute("INSERT INTO transactions (user_id, amount, type) VALUES (?, ?, 'admin_topup')", (user_id, amount))
        await db.commit()
        cur = await db.execute("SELECT balance FROM users WHERE id=?", (user_id,))
        new = (await cur.fetchone())[0]
    await update.message.reply_text(f"✅ Added ₹{amount} to user {target}\nNew balance: ₹{new}")

@admin_only
async def cmd_deductcoin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if len(context.args) < 2:
        await update.message.reply_text("Usage: /deductcoin <username_or_userid> <amount>")
        return
    target = context.args[0]
    try:
        amount = float(context.args[1])
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if current < amount:
            await update.message.reply_text(f"User has only ₹{current}, cannot deduct ₹{amount}.")
            return
        await db.execute("UPDATE users SET balance = balance - ? WHERE id=?", (amount, user_id))
        await db.execute("INSERT INTO transactions (user_id, amount, type) VALUES (?, ?, 'admin_deduction')", (user_id, -amount))
        await db.commit()
        cur = await db.execute("SELECT balance FROM users WHERE id=?", (user_id,))
        new = (await cur.fetchone())[0]
    await update.message.reply_text(f"✅ Deducted ₹{amount} from {target}\nNew balance: ₹{new}")
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid in PENDING_UPLOADS and PENDING_UPLOADS[uid].get("step") == "waiting_otp":
        await handle_otp_code(update, context)
        return

    # 4) phone entry step for admin upload
    if uid in CONFIG["ADMIN_IDS"] and context.user_data.get('upload_step') == 'phone':
...
async def callback_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "buy_accounts":
            ...
            await update.callback_query.answer("Use /broadcast <message> to send a broadcast.", show_alert=True)
        else:
...
async def main():
    ...
    global app
    app = ApplicationBuilder().token(CONFIG["BOT_TOKEN"]).request(http_request).build()
    ...
    scheduler.add_job(release_expired_reservations_tick, "interval", minutes=1, coalesce=True, max_instances=1)
    scheduler.start()
